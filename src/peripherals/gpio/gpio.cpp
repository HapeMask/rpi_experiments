...
GPIO::GPIO() {
    _virt_gpio_regs = map_phys_block((void*)(_asi.phys_mmio_base + GPIO_BASE_OFS), GPIO_LEN, _asi.page_size);

    for(int i=0; i < N_GPIO_SET_CLR_REGS; ++i) {
        _set_regs[i] = reg_addr(_virt_gpio_regs, GPIO_OUT_SET_OFS) + i;
        _clr_regs[i] = reg_addr(_virt_gpio_regs, GPIO_OUT_CLR_OFS) + i;
        _orig_set[i] = *_set_regs[i];
        _orig_clr[i] = *_clr_regs[i];
    }

    for(int i=0; i < N_GPIO_MODE_REGS; ++i) {
        _mode_regs[i] = reg_addr(_virt_gpio_regs, GPIO_MODE_OFS) + i;
        _orig_mode[i] = *_mode_regs[i];
    }

    for(int i=0; i < N_GPIO_LVL_REGS; ++i) {
        _lvl_regs[i] = reg_addr(_virt_gpio_regs, GPIO_LVL_OFS) + i;
        _orig_lvl[i] = *_lvl_regs[i];
    }
}
...
GPIO::~GPIO() {
    if (_virt_gpio_regs) {
        ...
        for(int i=0; i < N_GPIO_LVL_REGS; ++i) {
            *_lvl_regs[i] = _orig_lvl[i];
        }

        unmap_phys_block((void*)(_asi.phys_mmio_base + GPIO_BASE_OFS), GPIO_LEN, _asi.page_size);
    }
}
...
void* GPIO::reg_to_bus(uint32_t reg_ofs_bytes) const {
    return (void*)(_asi.bus_mmio_base + GPIO_BASE_OFS + reg_ofs_bytes);
}
